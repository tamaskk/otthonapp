...
const APIExercisesComponent = () => {
  ...
  const fetchExercises = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/api-exercise?${queryParams.toString()}`);
      ...
      const exercisesWithLinks: Exercise[] = data.results.map((exercise: any) => {
  ...
  const saveExercise = async (exerciseItem: Exercise) => {
    ...
    if (!exerciseItem.reps || !exerciseItem.sets || !exerciseItem.weight || !exerciseItem.rest_time || !exerciseItem.branded_video) {
  ...
  return (
    <div className="container mx-auto p-4 max-w-lg text-black">
      <h1 className="text-2xl font-bold mb-4 mt-4 text-center">Edzés keresése MuscleWiki-n</h1>
      <div className="flex flex-col space-y-6 bg-white p-6 rounded-lg shadow-lg border border-gray-100">
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-1">
            <label className="block text-sm font-medium text-gray-700 mb-2">Maximum eredmények száma</label>
            <input
              type="number"
              value={limit || ''}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Izmok</label>
          <select
            value={muscles}
          ...
          exercises.map((exercise) => (
              ...
              <h3 className="text-lg font-bold">{exercise.name}</h3>
            {
                exercise.branded_video && (
                    <div className="aspect-w-16 aspect-h-9">
                        <video src={exercise.branded_video} autoPlay muted loop />
                    </div>
                )
            }
            <div className='flex flex-row flex-wrap gap-2 justify-between w-full mt-4'>
                ...
                <input 
                    ...
                    onChange={(e) => {
                        const updatedExercises = exercises.map(ex => 
                            ex.id === exercise.id ? {...ex, rest_time: Number(e.target.value)} : ex
                        );
                        setExercises(updatedExercises);
                    }}
                />
            ...
            <button onClick={() => saveExercise(exercise)} className='bg-blue-500 w-full mt-2 text-white p-2 rounded hover:bg-blue-600 text-base'>
                Mentés
            </button>
            </div>
          ))
